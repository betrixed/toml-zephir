...
PHP_METHOD(Toml_Lexer, tokenName) {
	...
	zephir_array_fetch_long(&_2, &_1, tokenId, PH_NOISY | PH_READONLY, "toml/Lexer.zep", 104 TSRMLS_CC);
	RETURN_CTORW(&_2);
...
PHP_METHOD(Toml_Lexer, getExpSet) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&idList, 0, "toml/Lexer.zep", 118);
...
PHP_METHOD(Toml_Lexer, tokenize) {
	...
	while (1) {
		...
		zephir_array_append(&list, &_7$$3, PH_SEPARATE, "toml/Lexer.zep", 183);
	}
	...
	zephir_array_append(&list, &_9, PH_SEPARATE, "toml/Lexer.zep", 185);
...
void zephir_init_static_properties_Toml_Lexer(TSRMLS_D) {
	...
	ZEPHIR_INIT_VAR(&_5);
	zephir_create_array(&_5, 25, 0 TSRMLS_CC);
	ZEPHIR_INIT_NVAR(&_1);
	...
	zephir_array_fast_append(&_5, &_1);
	zend_update_static_property(toml_lexer_ce, ZEND_STRL("_nameList"), &_5);
	ZEPHIR_MM_RESTORE();
