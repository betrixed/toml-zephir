...
ZEPHIR_INIT_CLASS(Yosy_ValueList) {
	...
	zend_declare_property_null(yosy_valuelist_ce, SL("_list"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Yosy_ValueList, offsetSet) {
	...
	if (zephir_fast_count_int(&_0 TSRMLS_CC) == 0) {
		...
		if (!ZEPHIR_IS_IDENTICAL(&_1$$4, &atype)) {
			...
			object_init_ex(&_2$$5, yosy_xarrayable_ce);
			...
			zephir_throw_exception_debug(&_2$$5, "yosy/ValueList.zep", 30 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
...
PHP_METHOD(Yosy_ValueList, offsetGet) {
	...
	zephir_read_property(&_0, this_ptr, SL("_list"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_OBS_VAR(&_1);
	zephir_array_fetch_long(&_1, &_0, offset, PH_NOISY, "yosy/ValueList.zep", 46 TSRMLS_CC);
	RETURN_CCTOR(&_1);
...
PHP_METHOD(Yosy_ValueList, get) {
	...
	if (zephir_array_isset_long(&_0, offset)) {
		...
		zephir_array_fetch_long(&_2$$3, &_1$$3, offset, PH_NOISY | PH_READONLY, "yosy/ValueList.zep", 63 TSRMLS_CC);
		RETURN_CTORW(&_2$$3);
	} else {
...
PHP_METHOD(Yosy_ValueList, toArray) {
	...
	array_init(&result);
	zephir_read_property(&_0, this_ptr, SL("_list"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_0, 0, "yosy/ValueList.zep", 92);
