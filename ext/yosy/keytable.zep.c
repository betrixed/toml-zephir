...
ZEPHIR_INIT_CLASS(Yosy_KeyTable) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Yosy, KeyTable, yosy, keytable, yosy_arrayable_ce, yosy_keytable_method_entry, ZEND_ACC_FINAL_CLASS);

	zend_declare_property_null(yosy_keytable_ce, SL("_store"), ZEND_ACC_PUBLIC TSRMLS_CC);
...
PHP_METHOD(Yosy_KeyTable, __construct) {
	...
	if (!(Z_TYPE_P(&seed) == IS_NULL)) {
		zephir_is_iterable(&seed, 0, "yosy/KeyTable.zep", 27);
...
PHP_METHOD(Yosy_KeyTable, offsetGet) {
	...
	zephir_read_property(&_0, this_ptr, SL("_store"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch(&_1, &_0, index, PH_NOISY | PH_READONLY, "yosy/KeyTable.zep", 47 TSRMLS_CC);
	RETURN_CTORW(&_1);
...
PHP_METHOD(Yosy_KeyTable, get) {
	...
	if (zephir_array_isset(&_1, index)) {
		...
		zephir_array_fetch(&_0, &_2, index, PH_NOISY, "yosy/KeyTable.zep", 62 TSRMLS_CC);
	} else {
...
PHP_METHOD(Yosy_KeyTable, toArray) {
	...
	array_init(&result);
	zephir_read_property(&_0, this_ptr, SL("_store"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_0, 0, "yosy/KeyTable.zep", 84);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _2, _3, _1)
	{
		...
		_5$$3 = _4$$3;
		if (_5$$3) {
			_5$$3 = (zephir_instance_of_ev(&value, yosy_arrayable_ce TSRMLS_CC));
		}
		if (_5$$3) {
...
PHP_METHOD(Yosy_KeyTable, treeIterateValues) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_2), _4, _5, _3)
	{
		...
		if (_6$$4) {
			...
			ZVAL_STRING(&_7$$4, "\\Yosy\\KeyTable");
...
PHP_METHOD(Yosy_KeyTable, _merge) {
	...
	zephir_is_iterable(&_0, 0, "yosy/KeyTable.zep", 143);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _2, _3, _1)
	{
		ZEPHIR_INIT_NVAR(&key);
		if (_3 != NULL) { 
		...
		if (Z_TYPE_P(&myObj) == IS_OBJECT) {
			_5$$6 = (zephir_instance_of_ev(&myObj, yosy_keytable_ce TSRMLS_CC));
			if (_5$$6) {
				_5$$6 = Z_TYPE_P(&value) == IS_OBJECT;
			}
			_6$$6 = _5$$6;
			if (_6$$6) {
				_6$$6 = (zephir_instance_of_ev(&value, yosy_keytable_ce TSRMLS_CC));
			}
			if (_6$$6) {
